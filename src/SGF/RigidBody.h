...
class RigidBody : public Character {
   public:
    ...
    void setVelocity(const Vector2& velocity) { setVelocity((float)velocity.x, (float)velocity.y); }
    ...
    void applyForce(const Vector2& force) { applyForce((float)force.x, (float)force.y); }
